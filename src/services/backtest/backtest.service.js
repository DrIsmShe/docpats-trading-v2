...
const TRAILING_TRIGGER = 999; // отключён для теста
const TRAILING_DISTANCE = 1.0; // трейлинг = 1.0 ATR
const MAX_HOLD = 36; // было 48 — даём больше времени
...
export const backtest = (candles, strategy) => {
  ...
  for (let i = 60; i < candles.length - 1; i++) {
    ...
    if (position) {
      ...
      if (hitSL || hitTP || trailingHit || timeExit) {
        ...
        if (profit < 0) {
          cooldown = 2;
          lossStreak++;
          lastLossIndex = i; // запоминаем когда был лосс
        } else {
          lossStreak = 0;
        }
        trades.push({
    ...
    // Временная блокировка после серии лоссов (20 свечей паузы, потом сброс)
    if (lossStreak >= 4) {
      if (i - lastLossIndex > 20) {
        lossStreak = 0;
      } else {
        continue;
      }
